...
fn task5(input: []const u8, a: std.mem.Allocator) !usize {
    ...
    while (iter.next()) |line| {
        ...
        switch (state) {
            ...
            .updates => {
                var update = std.ArrayList(usize).init(a);
                defer update.deinit();
                ...
                if (isSorted(rulesList.items, update.items)) {
...
fn task5b(input: []const u8, a: std.mem.Allocator) !usize {
    var result: usize = 0;
    var iter = std.mem.splitScalar(u8, input, '\n');
    var state: ParseState = .rules;
    const rules = std.ArrayList([2]usize);
    ...
    while (iter.next()) |line| {
        ...
        switch (state) {
            ...
            .updates => {
                ...
                if (!isSorted(rulesList.items, update.items)) {
                    std.mem.sort(usize, update.items, rulesList.items, lessThanFn);
                    result += update.items[update.items.len / 2];
                }
            },
        }
...
fn lessThanFn(rules: [][2]usize, lhs: usize, rhs: usize) bool {
    const lr = [2]usize{ lhs, rhs };
    const rl = [2]usize{ rhs, lhs };
    for (rules) |rule| {
        if (std.mem.eql(usize, &lr, &rule)) {
            return true;
        }
        if (std.mem.eql(usize, &rl, &rule)) {
            return false;
        }
    }
...
fn isSorted(rules: [][2]usize, update: []usize) bool {
    for (rules) |rule| {
        const aIdx = std.mem.indexOfScalar(usize, update, rule[0]) orelse 0;
        const bIdx = std.mem.indexOfScalar(usize, update, rule[1]) orelse std.math.maxInt(usize);
...
test "task a" {
    const actual = try task5(testInput(), std.testing.allocator);
...
test "task a, final input" {
    const actual = try task5(finalText(), std.testing.allocator);
