...
const ParseStream = struct {
    in: []const u8,
    pos: usize = 0,

    fn init(in: []const u8) @This() {
        return .{
            .in = in,
            .pos = 0,
        };
    ...
    // Parse numbes out of sample input like:
    // Button A: X+26, Y+66
    // Button B: X+67, Y+21
    // Prize: X=12748, Y=12176
    fn parse(self: *@This()) !?[6]isize {
        var ret: [6]isize = [6]isize{ 0, 0, 0, 0, 0, 0 };
        var k: usize = 0;
        while (std.mem.indexOfAnyPos(u8, self.in, self.pos, "0123456789")) |i| {
            const iend = std.mem.indexOfNonePos(u8, self.in, i, "0123456789") orelse self.in.len;
            self.pos = iend;
            ret[k] = try std.fmt.parseInt(isize, self.in[i..iend], 10);
            k += 1;
            if (k == 6) {
                break;
            }
...
fn parse(in: []const u8) !?[6]isize {
    var ps = ParseStream.init(in);
    return ps.parse();
}

fn parseAndSumAll(in: []const u8) !isize {
    var ret: isize = 0;
    var ps = ParseStream.init(in);

    while (try ps.parse()) |p| {
        if (calc(p[0], p[1], p[2], p[3], p[4], p[5])) |price| {
            ret += price;
        }
    }
...
fn calc(ax: isize, ay: isize, bx: isize, by: isize, x: isize, y: isize) ?isize {
    const X = x + 10000000000000;
    const Y = y + 10000000000000;
    const b = @divTrunc(Y * ax - X * ay, by * ax - bx * ay);
    const a = @divTrunc(X - b * bx, ax);
...
test "1" {
    const in =
        ...
        \\Prize: X=8400, Y=5400
    ;
    const p = (try parse(in)).?;
    const price = calc(p[0], p[1], p[2], p[3], p[4], p[5]);
    try std.testing.expectEqual(null, price);
}
...
test "2" {
    ...
    const p = (try parse(in)).?;
    const price = calc(p[0], p[1], p[2], p[3], p[4], p[5]);
    try std.testing.expectEqual(459236326669, price);
}
...
test "3" {
    ...
    const actual = parseAndSumAll(in);
    try std.testing.expectEqual(875318608908, actual);
}
...
test "finalInput" {
    ...
    const actual = parseAndSumAll(in);
    try std.testing.expectEqual(96787395375634, actual);
}
...
//a*34 + b*67 = 5400
//
//(8400-b*22)*34/94 + b*67=5400 | *94
//(8400-b*22)*34 + b*67*94=5400*94
//8400*34-b*22*34 + b*67*94=5400*94
//b*67*94-b*22*34 = 5400*94 -8400*34
//b*(67*94-22*34) = 5400*94 -8400*34
